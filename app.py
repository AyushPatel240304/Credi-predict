...
def save_individual_prediction(data, result):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO individual_predictions (
                ...
                risk_probability, risk_level, decision
        ...
        """, (
            ...
            data['CNT_FAM_MEMBERS'], data['OCCUPATION_TYPE'], data['AGE_YEARS'],
            data['EMPLOYED_YEARS'], result['risk_probability'],
            result['risk_level'], result['recommendation']['decision']
        ))
        conn.commit()
        cursor.close()
        conn.close()
    except Exception as e:
        print(f"Error saving individual prediction: {e}")


def save_batch_prediction(results_df):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO batch_jobs (total_records, approved_count, rejected_count, review_count)
            VALUES (%s,%s,%s,%s) RETURNING id
        """, (
            len(results_df),
            len(results_df[results_df['Decision'] == 'APPROVED']),
            len(results_df[results_df['Decision'] == 'REJECTED']),
            len(results_df[results_df['Decision'] == 'MANUAL REVIEW'])
        ))
        ...
        for _, row in results_df.iterrows():
            cursor.execute("""
                INSERT INTO batch_predictions (
                    batch_job_id, code_gender, flag_own_car, flag_own_realty,
                    cnt_children, amt_income_total, name_income_type,
                    name_education_type, name_family_status, name_housing_type,
                    cnt_fam_members, occupation_type, age_years, employed_years,
                    risk_probability, risk_level, decision
            ...
            """, (
                batch_job_id,
                row['CODE_GENDER'], row['FLAG_OWN_CAR'], row['FLAG_OWN_REALTY'],
                row['CNT_CHILDREN'], row['AMT_INCOME_TOTAL'], row['NAME_INCOME_TYPE'],
                row['NAME_EDUCATION_TYPE'], row['NAME_FAMILY_STATUS'], row['NAME_HOUSING_TYPE'],
                row['CNT_FAM_MEMBERS'], row['OCCUPATION_TYPE'], row['AGE_YEARS'],
                row['EMPLOYED_YEARS'], row['Risk_Probability'],
...
def predict_single_route():
    ...
    try:
        ...
        save_individual_prediction(data, result)
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            if results_df is not None:
                save_batch_prediction(results_df)
